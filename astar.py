...
class Spot:
	def __init__(self, row, col, width, total_rows, barrier=WHITE):
		self.row = row
		self.col = col
		self.x = row * width
		self.y = col * width
...
def make_grid(rows, width, _grid):
	grid = []
	gap = width // rows
	for i in range(rows):
		grid.append([])
		for j in range(rows):
			spot = Spot(i, j, gap, rows, barrier=BLACK if _grid[j][i] > 0 else WHITE)
			grid[i].append(spot)
...
def main(grid, ROWS):
	...
	win = WIN
	width = WIDTH
	grid = make_grid(ROWS, width, grid)
